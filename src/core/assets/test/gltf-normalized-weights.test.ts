...
jest.mock('cc/editor/exotic-animation', () => ({
    exoticAnimationTag: Symbol('exoticAnimation'),
    RealArrayTrack: class RealArrayTrack { },
}));

jest.mock('cc/editor/color-utils', () => ({
    linearToSrgb8Bit: (value: number) => Math.round(value * 255),
}));

import { GltfConverter } from '../asset-handler/assets/utils/gltf-converter';
import { PPGeometry } from '../asset-handler/assets/utils/pp-geometry';
